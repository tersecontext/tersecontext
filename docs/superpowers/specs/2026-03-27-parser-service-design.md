...
extractor.py  (walk AST → nodes + CALLS edges)
        ...
        ├─ diff_type=modified     → filter to changed_nodes + added_nodes stable_ids
        ├─ diff_type=added        → emit all nodes
        └─ diff_type=deleted      → emit empty nodes[], forward deleted_nodes unchanged
        │
        ▼
...
- `ParsedFileEvent` — fields: `file_path`, `language`, `repo`, `commit_sha`, `nodes`, `intra_file_edges`
...
| Type | Tree-sitter node | `name` | `qualified_name` | `parent_id` |
|---|---|---|---|---|
...
**Per-node fields:**
- `body`: slice `source_bytes[node.start_byte:node.end_byte].decode()`
- `signature`: reconstruct from AST `parameters` + return type annotation; include type hints if present; empty string for imports/classes
- `docstring`: first `expression_statement` containing a string literal in the node body; empty string if absent
- `line_start` / `line_end`: `node.start_point[0] + 1` / `node.end_point[0] + 1` (1-indexed)
- `stable_id`: `"sha256:" + sha256(f"{repo}:{file_path}:{type}:{qualified_name}")`
- `node_hash`: `"sha256:" + sha256(name + signature + body)`
...
3. File with imports → import nodes present in output
4. Function with no docstring → `docstring` is `""`, not `None`
5. `diff_type=modified` → only emits nodes whose `stable_id` is in `changed_nodes + added_nodes`
6. `diff_type=deleted` → emits `ParsedFileEvent` with empty `nodes[]` and `intra_file_edges[]`
7. `stable_id` is deterministic — same input twice produces identical IDs
8. `node_hash` changes when function body changes
