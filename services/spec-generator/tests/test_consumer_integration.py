"""Integration tests for the spec-generator consumer loop (SpecGeneratorConsumer).
...
STREAM_IN = SpecGeneratorConsumer.stream
GROUP = SpecGeneratorConsumer.group
...
async def test_consumer_creates_group_on_startup():
    ...
    with patch("shared.consumer.aioredis.from_url", return_value=mock_redis):
        with pytest.raises(asyncio.CancelledError):
            await SpecGeneratorConsumer(mock_store).run("redis://localhost:6379")
...
async def test_consumer_processes_execution_path_message():
    ...
    consumer_mod.messages_processed_total = 0
    ...
    assert consumer_mod.messages_processed_total == 1
...
async def test_consumer_handles_malformed_message_and_routes_to_dlq():
    """Consumer sends malformed messages (bad JSON / ValueError) to DLQ and ACKs them."""
    import app.consumer as consumer_mod
    ...
    msg_id = b"4-0"
    # bad JSON causes json.JSONDecodeError which is a ValueError subclass
    ...
    # Bad JSON (ValueError) → DLQ route → ACKed
    mock_redis.xack.assert_called_once_with(STREAM_IN, GROUP, msg_id)
    assert consumer_mod.messages_processed_total == 0
    mock_store.upsert_spec.assert_not_called()


async def test_consumer_handles_validation_error_routes_to_dlq():
    """Consumer routes messages that fail Pydantic validation to DLQ.

    Pydantic's ValidationError is a subclass of ValueError, so RedisConsumerBase
    catches it in the (KeyError, ValueError) branch → DLQ + ACK.
    """
    import app.consumer as consumer_mod
    ...
    msg_id = b"5-0"
    # Valid JSON but bad schema — raises ValidationError (a ValueError subclass)
    ...
    # ValidationError (ValueError subclass) → DLQ route → ACKed
    mock_redis.xack.assert_called_once_with(STREAM_IN, GROUP, msg_id)
    mock_redis.xadd.assert_called_once()  # DLQ write
    assert consumer_mod.messages_failed_total == 0  # counter not touched by handle() or base
...
async def test_consumer_handles_missing_event_key_routes_to_dlq():
    """Consumer routes messages with missing 'event' key to DLQ (KeyError)."""
    import app.consumer as consumer_mod
